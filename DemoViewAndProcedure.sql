...
CREATE PROCEDURE  PRO_TONG_VATTU (IN MAVATTU VARCHAR(255))
BEGIN
SELECT (K.SOLUONGDAU + K.TONGSOLUONGNHAP-K.TONGSOLUONGXUAT) AS 'TON CUOI' FROM VATLIEU JOIN KHO K on VATLIEU.ID = K.VATTU_ID
WHERE MA_VATLIEU = MAVATTU;
end;

CALL PRO_TONG_VATTU('VL03');

# Câu 2. Tạo SP cho biết tổng tiền xuất của vật tư với mã vật tư là tham số vào.
CREATE PROCEDURE PRO_TONG_TIEN(IN MAVATTU VARCHAR(255))
BEGIN
SELECT TEN_VATLIEU AS 'TEN VAT LIEU' ,SUM(SOLUONGXUAT*C.DONGIAXUAT) AS 'TONG TIEN'
FROM VATLIEU JOIN CTPHIEUXUAT C on VATLIEU.ID = C.VATTU_ID
WHERE MA_VATLIEU = MAVATTU
GROUP BY MA_VATLIEU;
end;

CALL PRO_TONG_TIEN('VL04');


# Câu 3. Tạo SP cho biết tổng số lượng đặt theo số đơn hàng với số đơn hàng là tham số vào.
CREATE PROCEDURE PRO_TONG_SOLUONGDAT(IN SO_DONHANG VARCHAR(255))
BEGIN
SELECT MA_DONHANG AS 'MA DON HANG', SUM(SOLUONGDAT) AS 'TONG SO LUONG DAT'
FROM CTDONHANG JOIN DONHANG D on D.ID = CTDONHANG.DONHANG_ID
WHERE MA_DONHANG = SO_DONHANG
GROUP BY MA_DONHANG;
end;

CALL PRO_TONG_SOLUONGDAT('MH03');


# Câu 4. Tạo SP dùng để thêm một đơn đặt hàng.
CREATE PROCEDURE PRO_THEM_DONDATHANG(IN MADONHANG VARCHAR(255),NGAYDAT DATE, ID_NHACUNGCAP INT)
BEGIN
INSERT INTO DONHANG(MA_DONHANG, NGAYDATHANG, NHACUNGCAP_ID)
VALUES (MADONHANG,NGAYDAT,ID_NHACUNGCAP);
end;

CALL PRO_THEM_DONDATHANG('MH04','2021-05-21',3);

# Câu 5. Tạo SP dùng để thêm một chi tiết đơn đặt hàng.
CREATE PROCEDURE THEM_DONHANG_CHITIET(IN ID_DONHANG INT, ID_VATTU INT, SOLUONG INT)
BEGIN
INSERT INTO CTDONHANG(DONHANG_ID, VATTU_ID, SOLUONGDAT)
VALUES (ID_DONHANG,ID_VATTU,SOLUONG);
end;

CALL THEM_DONHANG_CHITIET(2,3,400);